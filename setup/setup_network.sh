...
# Variables
SSID="${ROCKTIMER_SSID:-stonetimer}"
...
if [ "${ENABLE_INTERNET_SHARING}" = "1" ] && [ "${uplink_ok}" = "1" ]; then
  ...
  cat > /etc/sysctl.d/99-rocktimer-ipforward.conf << EOF
...
EOF
  ...
  cat > /etc/nftables.d/rocktimer.nft << EOF
# RockTimer NAT + forwarding (generated by setup_network.sh)
table inet rocktimer_filter {
  chain forward {
...
EOF
    ...
    cp -n /etc/nftables.conf /etc/nftables.conf.rocktimer.bak || true
    printf '\ninclude "/etc/nftables.d/*.nft"\n' >> /etc/nftables.conf
  fi
