#!/bin/bash
# Network configuration for SplitStone
# Run on Pi 4 to set up a WiFi Access Point

set -e

echo "==================================="
echo "SplitStone Network Configuration"
echo "==================================="
...
# Variables
SSID="${SPLITSTONE_SSID:-splitstone}"
PASSWORD="${SPLITSTONE_PASSWORD:-splitstone}"
IP_ADDRESS="${SPLITSTONE_IP_ADDRESS:-192.168.50.1}"
SUBNET_CIDR="${SPLITSTONE_SUBNET_CIDR:-192.168.50.0/24}"
AP_INTERFACE="${SPLITSTONE_AP_INTERFACE:-wlan0}"
# Wi-Fi regulatory domain / country code for hostapd (improves client compatibility)
COUNTRY_CODE="${SPLITSTONE_COUNTRY_CODE:-SE}"
# Internet sharing (NAT) is enabled by default so Pi Zero clients can reach the internet
# via the Pi 4 uplink (typically eth0). Set SPLITSTONE_ENABLE_INTERNET_SHARING=0 to disable.
ENABLE_INTERNET_SHARING="${SPLITSTONE_ENABLE_INTERNET_SHARING:-1}"
# Uplink interface used for internet access when sharing is enabled (typically eth0 if Pi 4 is wired)
UPLINK_INTERFACE="${SPLITSTONE_UPLINK_INTERFACE:-eth0}"
CH_DHCPCD_BEGIN="# SplitStone AP begin"
CH_DHCPCD_END="# SplitStone AP end"
...
EOF
    ...
    if systemctl is-active --quiet NetworkManager 2>/dev/null; then
        ...
        cat > /etc/NetworkManager/conf.d/99-splitstone-unmanaged-wlan0.conf << EOF
[keyfile]
...
EOF
    ...
    cat > /etc/systemd/network/99-splitstone-${AP_INTERFACE}.network << EOF
[Match]
Name=${AP_INTERFACE}
...
# DHCP range for SplitStone network
dhcp-range=192.168.50.10,192.168.50.100,255.255.255.0,24h

# Advertise Pi 4 as the default gateway for clients
dhcp-option=option:router,${IP_ADDRESS}

# Make the hostname 'splitstone' resolve to the Pi 4 (this server)
address=/splitstone/${IP_ADDRESS}

# Optional: also support splitstone.local
address=/splitstone.local/${IP_ADDRESS}
...
# Provide a search domain so some clients can type http://splitstone
dhcp-option=option:domain-name,splitstone
dhcp-option=option:domain-search,splitstone
EOF
...
if [ "${ENABLE_INTERNET_SHARING}" = "1" ] && [ "${uplink_ok}" = "1" ]; then
  ...
  cat > /etc/sysctl.d/99-splitstone-ipforward.conf << EOF
...
EOF
  ...
  cat > /etc/nftables.d/splitstone.nft << EOF
# SplitStone NAT + forwarding (generated by setup_network.sh)
table inet splitstone_filter {
  chain forward {
...
EOF
    ...
    cp -n /etc/nftables.conf /etc/nftables.conf.splitstone.bak || true
    printf '\ninclude "/etc/nftables.d/*.nft"\n' >> /etc/nftables.conf
  fi
...
if [ "${ENABLE_INTERNET_SHARING}" = "1" ]; then
  ...
  echo "Internet sharing: disabled (set SPLITSTONE_ENABLE_INTERNET_SHARING=1 to enable)"
fi
echo ""
