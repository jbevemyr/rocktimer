...
def _load_server_from_config(config_path: Path) -> Optional[Tuple[str, int, int]]:
    """Load (host, udp_port, http_port) from a SplitStone config.yaml if available."""
    if yaml is None:
        return None
...
def arm_server(server_host: str, http_port: int, timeout_s: float = 1.0) -> bool:
    """Best-effort: ask the SplitStone server to arm (rearm) via HTTP."""
...
def main():
    parser = argparse.ArgumentParser(description="Simulate SplitStone triggers via UDP")
    ...
    parser.add_argument("--config", default=None, help="Path to config.yaml (default: ./config.yaml, then /opt/splitstone/config.yaml)")
    ...
    if args.config:
        ...
        config_candidates.extend([Path("config.yaml"), Path("/opt/splitstone/config.yaml")])
