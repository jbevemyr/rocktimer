#!/bin/bash
# Install SplitStone Sensor on Raspberry Pi Zero 2 W

set -e

echo "==================================="
echo "SplitStone Sensor Installation"
echo "==================================="
...
CHRONY_SERVER="${SPLITSTONE_CHRONY_SERVER:-${CHRONY_SERVER_DEFAULT}}"
...
if [[ "${configure_chrony}" =~ ^[Yy]$ ]]; then
    if [ ! -f "${CHRONY_CONF}" ]; then
        ...
        if [ -z "${SPLITSTONE_CHRONY_SERVER:-}" ]; then
...
${CHRONY_MARKER_BEGIN}
# Prefer SplitStone Pi 4 as time source
...
cat > /etc/systemd/system/splitstone-sensor.service << EOF
[Unit]
Description=SplitStone Sensor Daemon
...
systemctl enable splitstone-sensor.service
...
echo "  sudo systemctl start splitstone-sensor"
echo ""
echo "Check status:"
echo "  sudo systemctl status splitstone-sensor"
echo ""
echo "View logs:"
echo "  sudo journalctl -u splitstone-sensor -f"
echo ""
